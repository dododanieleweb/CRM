    ...
    <main className="grid min-h-screen place-items-center bg-background p-4 text-foreground">
      <section className="w-full max-w-md rounded-lg border bg-card p-6 shadow-soft">
        ...
        {!configured ? <p className="rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-600">Supabase non e configurato. Imposta le variabili d'ambiente prima del deploy.</p> : <form onSubmit={submit} className="grid gap-4">
          ...
          <label className="grid gap-1.5 text-sm font-medium">Password<input required name="password" type="password" minLength={8} autoComplete={mode === "login" ? "current-password" : "new-password"} className="h-10 rounded-lg border bg-background px-3 outline-none focus:border-primary" /></label>
          {message && <p className="rounded-lg border bg-muted p-3 text-sm text-muted-foreground">{message}</p>}
          <button disabled={loading} className="h-10 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50">{loading ? "Attendi..." : mode === "login" ? "Accedi" : "Crea account"}</button>
          <button type="button" onClick={() => { setMode((current) => current === "login" ? "signup" : "login"); setMessage(""); }} className="text-sm font-semibold text-primary">{mode === "login" ? "Crea un nuovo account" : "Hai gia un account? Accedi"}</button>
        </form>}
      </section>
    </main>
  );
